...
- Known remaining inconsistencies: engine-resolver.ts has 22 static Logger.debug/warning calls (static class, hard to refactor); project-initializer.ts has 9 static Logger calls; build-executor.ts has 1 stray static Logger.debug in static method getAvailableTargets; project-path-resolver.ts has 1 static Logger.warning; validator.ts has 2 static Logger.debug; error.ts handleCommandError has 2 static Logger.error (acceptable — calls process.exit immediately); cli/index.ts has 1 static Logger.error (acceptable — top-level fatal handler).
- Fixed: gencodebase.ts now uses instance Logger with stdout/stderr support (was 4 static calls); clean.ts now passes stdout/stderr to Logger constructor (was only passing silent); version.ts getVersionInfo() now receives logger parameter (was 1 stray static Logger.debug call).
